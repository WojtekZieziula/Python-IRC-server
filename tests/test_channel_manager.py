...
def test_normalization(channel_manager: ChannelManager) -> None:
    assert channel_manager._normalize_name("Polska") == "#polska"
    assert channel_manager._normalize_name("#POLSKA") == "#polska"


def test_create_channel_success(channel_manager: ChannelManager) -> None:
...
def test_create_duplicate_channel_fails(channel_manager: ChannelManager) -> None:
    channel_manager.create_channel("#test")
    with pytest.raises(ValueError, match="already exists"):
        channel_manager.create_channel("#TEST")


def test_create_invalid_channel_name_fails(channel_manager: ChannelManager) -> None:
...
def test_get_or_create_behavior(channel_manager: ChannelManager) -> None:
...
def test_remove_user_from_all_channels_and_cleanup(
    channel_manager: ChannelManager, mock_session: MagicMock
) -> None:
