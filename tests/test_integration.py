...
class IRCClient:
    ...
    async def wait_for_message(self, expected_substring, timeout=2.0):
        end_time = asyncio.get_running_loop().time() + timeout
        
        while True:
            time_left = end_time - asyncio.get_running_loop().time()
            if time_left <= 0:
                raise TimeoutError(f"[{self.nick}] Timeout - oczekiwano '{expected_substring}'")
            
            try:
                line = await asyncio.wait_for(self.reader.readline(), timeout=time_left)
                
                if not line:
                    raise ConnectionResetError(f"[{self.nick}] Serwer zamknął połączenie")
...
@pytest.mark.asyncio
async def test_integration_chat_between_users(running_server):
    port = running_server
...
@pytest.mark.asyncio
async def test_integration_kick_scenario(running_server):
    port = running_server
...
@pytest.mark.asyncio
async def test_integration_connect_and_quit(running_server):
    port = running_server
