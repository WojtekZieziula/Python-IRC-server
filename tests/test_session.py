...
@pytest.mark.asyncio
async def test_session_init(mock_streams, server_name):
    ...
    expected_logger_name = f"Session(192.168.1.12:12345)"
...
@pytest.mark.asyncio
async def test_session_init_unknown_address(mock_streams, server_name):
    mock_reader, mock_writer = mock_streams
...
@pytest.mark.asyncio
async def tests_session_send_reply_success(mock_streams, server_name):
    mock_reader, mock_writer = mock_streams
...
@pytest.mark.asyncio
async def test_session_send_reply_unicode(mock_streams, server_name):
    mock_reader, mock_writer = mock_streams
    session = ClientSession(mock_reader, mock_writer, server_name)
...
@pytest.mark.asyncio
async def test_session_prevents_sending_on_closed_connection(mock_streams, server_name):
...
@pytest.mark.asyncio
async def test_session_send_reply_network_error(mock_streams, server_name, caplog):
    mock_reader, mock_writer = mock_streams
    session = ClientSession(mock_reader, mock_writer, server_name)
...
@pytest.mark.asyncio
async def test_session_send_error_format(mock_streams, server_name):
    mock_reader, mock_writer = mock_streams
...
@pytest.mark.asyncio
async def test_session_quit_cleans_up(mock_streams, server_name):
    mock_reader, mock_writer = mock_streams
    session = ClientSession(mock_reader, mock_writer, server_name)
...
@pytest.mark.asyncio
async def test_session_quit_idempotency(mock_streams, server_name):
    mock_reader, mock_writer = mock_streams
    session = ClientSession(mock_reader, mock_writer, server_name)
...
@pytest.mark.asyncio
async def test_session_quit_handles_exception(mock_streams, server_name):
    mock_reader, mock_writer = mock_streams
    session = ClientSession(mock_reader, mock_writer, server_name)
